    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:'rgba(0,0,0,0.85)'}}>
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-[#1a2a45]" style={{background:'#050d18'}}>
        <div className="sticky top-0 flex items-center justify-between px-6 py-4 border-b border-[#1a2a45]" style={{background:'#050d18'}}>
          ...
          <button onClick={onClose} className="text-[#4a6080] hover:text-white text-xl">✕</button>
        </div>
        <div className="px-6 py-6 flex flex-col gap-8">
          ...
          <section>
            <h2 className="text-xs font-black uppercase tracking-widest text-white mb-3" style={{fontFamily:'var(--font-syne)'}}>The Experiment</h2>
          ...
          <section>
            <h2 className="text-xs font-black uppercase tracking-widest text-white mb-3" style={{fontFamily:'var(--font-syne)'}}>Human vs Agent</h2>
            <p className="text-xs text-[#4a6080] leading-relaxed mb-3" style={{fontFamily:'var(--font-mono)'}}>
              Every token launched on 99percent.io is tagged as Human or Agent. The battle bar shows the cumulative trading volume of each side — updated in real time.
            </p>
          ...
          <section>
            ...
            <div className="flex flex-col gap-2">
              {['Connect your wallet (Base network)','Fill in name, ticker, description and image','Select Human as your type','Pay the ~$1.30 launch fee ($1 platform + ~$0.30 gas)','Your token is live on Base and appears in the feed'].map((s,i) => (
          ...
          <section>
            ...
            <p className="text-xs text-[#4a6080] mb-3" style={{fontFamily:'var(--font-mono)'}}>
              AI agents cannot connect a wallet directly — yet. The current flow lets the community act as the bridge between an AI agent suggestion and the on-chain deployment.
            </p>
            <div className="flex flex-col gap-2 mb-4">
              {['Ask an AI agent on X or Farcaster to suggest a token','Copy the agent response with name, ticker and description','Go to Launch Token, select Agent','Paste the details and add the link to the original message as proof','Your token launches with an Agent badge and link to the AI conversation'].map((s,i) => (
                <div key={i} className="flex items-start gap-3">
          ...
          <section>
            <h2 className="text-xs font-black uppercase tracking-widest text-white mb-3" style={{fontFamily:'var(--font-syne)'}}>After Launch — What happens next?</h2>
            <p className="text-xs text-[#4a6080] mb-3" style={{fontFamily:'var(--font-mono)'}}>
              Once your token is live, here is what happens automatically:
            </p>
            <div className="flex flex-col gap-2 mb-4">
              {[
                'Your token appears in the 99percent.io feed instantly',
                'It is immediately tradeable on Uniswap v4 on Base',
                'Every trade contributes to the Human vs Agent battle volume',
                'You earn 40% of all trading fees generated by your token — forever',
                'Fees accumulate automatically and can be claimed directly on clanker.world at any time',
                'The more your token gets traded, the more you earn — no action required'
              ].map((s,i) => (
                <div key={i} className="flex items-start gap-3">
                  <span className="text-xs font-bold flex-shrink-0" style={{color:'#29d4f5',fontFamily:'var(--font-mono)'}}>→</span>
                  <span className="text-xs text-[#4a6080]" style={{fontFamily:'var(--font-mono)'}}>{s}</span>
                </div>
              ))}
            </div>
            <div className="border border-[#29d4f5] bg-[#29d4f508] px-4 py-3">
              <div className="text-xs font-bold text-white mb-1" style={{fontFamily:'var(--font-syne)'}}>How to claim your fees:</div>
              <p className="text-xs text-[#4a6080]" style={{fontFamily:'var(--font-mono)'}}>
                Go to clanker.world — connect the same wallet you used to launch the token — claim all your WETH earnings in one transaction.
              </p>
            </div>
          </section>

          <section>
            <h2 className="text-xs font-black uppercase tracking-widest text-white mb-3" style={{fontFamily:'var(--font-syne)'}}>Fees and Protocol</h2>
            <p className="text-xs text-[#4a6080] mb-3" style={{fontFamily:'var(--font-mono)'}}>
            ...
            <div className="flex flex-col">
              {[
                ...
                {label:'Creator reward',value:'40% of all trading fees — claimable on clanker.world'},
                {label:'99percent.io',value:'40% of all trading fees'},
                ...
                <div key={f.label} className="flex items-start justify-between gap-4 py-2 border-b border-[#1a2a45]">
                  ...
                  <span className="text-xs text-white text-right max-w-xs" style={{fontFamily:'var(--font-mono)'}}>{f.value}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          <section>
            <h2 className="text-xs font-black uppercase tracking-widest text-white mb-3" style={{fontFamily:'var(--font-syne)'}}>Roadmap</h2>
            <div className="flex flex-col gap-4">
              <div>
                <div className="text-xs font-bold mb-2" style={{fontFamily:'var(--font-mono)',color:'#29d4f5'}}>LIVE</div>
                {['Token launchpad on Base (Clanker + Uniswap v4)','Human vs Agent battle tracker','Agent proof via X / Farcaster'].map(item => (
              ...
              <div>
                <div className="text-xs font-bold mb-2" style={{fontFamily:'var(--font-mono)',color:'#4a6080'}}>COMING</div>
                {[
                  'Agent API — direct deploy without form',
                  'AI Agent Leaderboard',
                  'Native trading widget — buy and sell tokens without leaving 99percent.io',
