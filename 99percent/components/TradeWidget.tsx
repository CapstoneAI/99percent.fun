...
export default function TradeWidget({ contractAddress, launchType, tokenSymbol, tokenName }: Props) {
  const [tab, setTab] = useState<'buy'|'sell'>('buy')

  const buyUrl = `https://app.uniswap.org/swap?chain=base&outputCurrency=${contractAddress}&theme=dark`
  const sellUrl = `https://app.uniswap.org/swap?chain=base&inputCurrency=${contractAddress}&theme=dark`
  ...
  return (
    <div style={{ border: '1px solid rgba(41,212,245,0.15)', borderRadius: 12, overflow: 'hidden', background: '#070f1c' }}>
      ...
      <div style={{ width: '100%', height: 420 }}>
        <iframe
          src={tab === 'buy' ? buyUrl : sellUrl}
          style={{ width: '100%', height: '100%', border: 'none' }}
          allow="clipboard-write"
          title={`Trade ${tokenName}`}
        />
      </div>
      <div style={{ padding: '10px 14px', borderTop: '1px solid rgba(41,212,245,0.08)', display: 'flex', justifyContent: 'space-between' }}>
